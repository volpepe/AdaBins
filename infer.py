...
class InferenceHelper:
    def __init__(self, dataset='nyu', device='cuda:0', pretrained_dir='./pretrained'):
        ...
        if dataset == 'nyu':
            ...
            pretrained_path = os.path.join(pretrained_dir, "AdaBins_nyu.pt")
        elif dataset == 'kitti':
            self.min_depth = 1e-3
            self.max_depth = 80
            self.saving_factor = 256
            model = UnetAdaptiveBins.build(n_bins=256, min_val=self.min_depth, max_val=self.max_depth)
            pretrained_path = os.path.join(pretrained_dir, "AdaBins_kitti.pt")
        else:
